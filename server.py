...
def remember_ingredient(
    ...
    """Saves an ingredient to the user's personal AVOID(트러블) or PREFER(잘 맞음) list in SkinHarmony(스킨하모니), so every future report highlights it (★avoid / 👍prefer). Ask the user for a short nickname once and pass it as user_key. Idempotent: saving the same ingredient again just updates it.
...
def search_cosmetic_info(query_name: str, user_key: str | None = None) -> str:
    """Looks up a single cosmetic ingredient in SkinHarmony(스킨하모니)'s standard dictionary built from Korean MFDS(식약처) public data: standard Korean/English name, category, usage limits, and regulation flags. Also shows the user's saved preference for it if user_key is given. Returns "not found" honestly when there is no match — never fabricates data.
